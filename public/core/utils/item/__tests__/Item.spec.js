...
const TestClass2 = require('./TestClass2');
  ...
  describe(`Item.clone`, () => {
    context(`when called on an instance of a class that extends item`, () => {
      ...
      it(`the cache of the new object is reset (but not of the source)`, () => {
        ...
        expect(test1._cachedValueByValueKey).to.not.eql({});
        const copy = test1.clone();
        expect(test1._cachedValueByValueKey).to.not.eql({});
        expect(copy._cachedValueByValueKey).to.eql({});
      });
      it(`any key named "temp" is not cloned`);
    });
    context(`when called on an instance of a class that has a prop that extends collection`, () => {
      /** @type {TestClass2} */
      let test2;
      beforeEach(() => {
