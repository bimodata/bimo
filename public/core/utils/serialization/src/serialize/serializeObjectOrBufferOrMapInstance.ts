...
const KEYS_TO_IGNORE = new Set([
  "temp",
  "_cachedValueByValueKey",
  "ItemConstructor",
  "EntityConstructor",
  "_links",
  "policy",
  "_context",
  "_abstract", // Todo #29 review policy
  "_placesByReferencePlace",
  "_rawOirProps",
  "vehicleTask",
]);

export default function serializeObjectOrBufferOrMapInstance(
  objectOrBufferOrMapInstance: any,
  dataByConstructorName: Map<string, any>,
  linkIsAggregation = false,
  options: { allowUnknownAggregatedInstances?: any; treatAggregationAsComposition?: any }
) {
  ...
  if (linkIsAggregation && !treatAggregationAsComposition) {
    ...
    return {
      _type: objectOrBufferOrMapInstance.constructor.name,
      _repoId: objectOrBufferOrMapInstance.id,
    };
  }

  // 1 get the metaobjects that relate to this type of object
  let dataForThisConstructor = dataByConstructorName.get(
    objectOrBufferOrMapInstance.constructor.name
  );
  if (!dataForThisConstructor) {
    dataForThisConstructor = {
      nextId: 1,
      idByInstance: new WeakMap(),
      serializedInstanceById: {},
    };
    dataByConstructorName.set(
      objectOrBufferOrMapInstance.constructor.name,
      dataForThisConstructor
    );
  }
  ...
  if (!idOfThisObjectOrMapInstance) {
    ...
    let serializedVersionOfThisObjectOrBufferOrMapInstance: any;

    if (objectOrBufferOrMapInstance.constructor.name === "Map") {
      const entries = Array.from(objectOrBufferOrMapInstance.entries());
      serializedVersionOfThisObjectOrBufferOrMapInstance = entries.map((value) =>
        serializeArrayOrObjectOrMapOrValue(value, dataByConstructorName, false, options)
      );
    } else if (objectOrBufferOrMapInstance.constructor.name === "Set") {
      const values = Array.from(objectOrBufferOrMapInstance);
      serializedVersionOfThisObjectOrBufferOrMapInstance = values.map((value) =>
        serializeArrayOrObjectOrMapOrValue(value, dataByConstructorName, false, options)
      );
    } else if (objectOrBufferOrMapInstance.constructor.name === "Buffer") {
      serializedVersionOfThisObjectOrBufferOrMapInstance = Array.from(
        objectOrBufferOrMapInstance
      );
    } else if (
      objectOrBufferOrMapInstance.associationType &&
      ...
      Object.keys(objectOrBufferOrMapInstance).forEach((key) => {
        ...
        if (key === "items") {
          serializedVersionOfThisObjectOrBufferOrMapInstance[key] =
            serializeArrayOrObjectOrMapOrValue(
              value,
              dataByConstructorName,
              true,
              options
            );
    ...
    serializedInstanceById[idOfThisObjectOrMapInstance] =
      serializedVersionOfThisObjectOrBufferOrMapInstance;
  }
  ...
  return {
    _type: objectOrBufferOrMapInstance.constructor.name,
    _id: idOfThisObjectOrMapInstance,
  };
}

const primitives = new Set(["Number", "String", "Boolean", "Function"]);

function serializeArrayOrObjectOrMapOrValue(
  arrayOrObjectOrMapOrValue:any,
  dataByConstructorName:Map<string, any>,
  linkIsAggregation = false,
  options:any
) {
  if (
    !arrayOrObjectOrMapOrValue ||
    primitives.has(arrayOrObjectOrMapOrValue.constructor.name)
  ) {
    return arrayOrObjectOrMapOrValue;
  }
  if (arrayOrObjectOrMapOrValue.constructor.name === "Array") {
    return arrayOrObjectOrMapOrValue.map((value:any) =>
      serializeArrayOrObjectOrMapOrValue(
        value,
        dataByConstructorName,
        linkIsAggregation,
  ...
  return serializeObjectOrBufferOrMapInstance(
    arrayOrObjectOrMapOrValue,
    dataByConstructorName,
    linkIsAggregation,
    options
