...
const TripClassFactory = ({
  ...
  class Trip extends BlockActivityItemMixin(
    TripOrVariant, {
    ...
    copy(newInternalNumber) {
      ...
      /**
       * Think of the line below as const copiedTrip = new Trip(props);
       * But if we wrote it like this, the copy could ever only be a plain trip, even when we are copying from
       * a custom-extend instance of a trip. So first, we get the actual constructor of the current instance
       * (It will be the Plain trip in most cases, but if we created a CustomTrip that extends Trip, we now get CustomTrip)
       */
      // @ts-ignore
      const copiedTrip = new this.constructor(props);
...
module.exports = TripClassFactory;
