...
export function NetworkEdgeClassFactory({
  NetworkNode,
  Network,
  NetworkSection,
}: EntityConstructorByEntityClassKey): typeof BimoNetworkEdge {
  ...
  class NetworkEdge extends Item<NetworkEdge> {
    ...
    get sortedNodeIdsKey() {
      return this._getAndSetCachedValue("sortedNodeIdsKey", () => {
        ...
        if (
          parseInt(this.fromNode.businessId, 10) > parseInt(this.toNode.businessId, 10)
        ) {
...
export default NetworkEdgeClassFactory;
