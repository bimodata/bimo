...
export interface RouteProps extends ExtendedItemProps {
  ...
  variants: BimoVariantsCollection;
}
export function RouteClassFactory({
  VariantsCollection,
  RouteVersion,
}: EntityConstructorByEntityClassKey): typeof BimoRoute {
  const childClasses: (typeof Entity)[] = [VariantsCollection];

  class Route extends Item<Route> {
    bimoId?: string;
    rteVersion?: string;
    rteIdentifier?: string;
    _rteDescription?: string;
    ...
    get routeVersion() {
      return this.parent && (this.parent.parent as BimoRouteVersion);
    }
  }
...
export default RouteClassFactory;
